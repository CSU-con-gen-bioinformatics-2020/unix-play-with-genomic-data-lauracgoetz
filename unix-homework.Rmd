...
```{bash}
cd fastq
cp *.R1.fq.gz /Volumes/GOETZ/Course_work/advanced_computing/unix-play-with-genomic-data-lauracgoetz/lonely-fastqs/R1 

cp *.R2.fq.gz /Volumes/GOETZ/Course_work/advanced_computing/unix-play-with-genomic-data-lauracgoetz/lonely-fastqs/R2
```
...
```{bash}
cd fastq
cp DPCh_plate1_[A,B]* /Volumes/GOETZ/Course_work/advanced_computing/unix-play-with-genomic-data-lauracgoetz/fq_AB
```
...
```{bash}
cp DPCh_plate1_[^A,B]* /Volumes/GOETZ/Course_work/advanced_computing/unix-play-with-genomic-data-lauracgoetz/fq_notAB
```
...
```{bash}
mkdir nice_dir_name 
echo "A string in a file :D" > nice_dir_name/nice_file_name
```
...
echo "No spaces please :'(" > bad\ directory\ name\ with\ spaces/bad\ file\ name\ with\ spaces
```
...
```
What if there's something in there that you somehow forgot about?
```
...
```{bash}
mkdir A05_S5
cp {fastq,bam,sam}/DPCh_plate1_[A05_S5]* /Volumes/GOETZ/Course_work/advanced_computing/unix-play-with-genomic-data-lauracgoetz/A05_S5
```
...
```
Lines 1 and 5 are identical between files, with exception of /# at end which corresponds to R#
```
...
```{bash}
gzip R1_all_via_gzcat.fq > R1_all_via_gzipped.fq.gz
shasum R1_all_via_gzipped.fq.gz copy_of_R1_all_via_cat.fq.gz
```
...
Aha! You should have gotten a permission denied error (i didnt >:D hehe).  99% of the time that occurs because
...
```{bash}
# what are names supposed to match?
```
